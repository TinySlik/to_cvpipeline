	...
	*/
	...
	public:
		...
		*/
		...
		std::vector<std::function<bool(Config& dst, const Config& in)>> func_array;
		...
		bool is_hiden()         const { return _hiden;			  }
		void set_hiden(bool hid_in)   { _hiden = hid_in;		  }
		...
		///tiny oh add for 2 config value change
		static bool deep_async(Config& a, const Config& b);

		///tiny oh add for 2 config diff show
		static Config diff(const Config& a, const Config& b);
	...
	/// custom hidden feature, filter the unessary things not to show to the user.
	std::string dump_string_with_hiden(const Config& config, const FormatOptions& options);
...
namespace configuru
{
	...
	bool Config::judge_or_create_key(const std::string& key)
	{
		if (has_key(key.c_str()) && (*this)[key.c_str()].is_object())
		{
		    return true;
		} else {
		    (*this)[key.c_str()] = object();
		    return false;
		}
	}
	...
	bool Config::deep_eq(const Config& a, const Config& b)
	{
		...
		if (a._type == Int && b._type == Float) {
		  Config tmp = a;
		  tmp._type = Float;
		  tmp._u.f = (float)(a._u.i);
		  if (abs(tmp._u.f - b._u.f) > 1e-6) {
		    return false;
		  } else {
		    return true;
		  }
		}
		if (a._type == Float && b._type == Int) {
		  Config tmp = b;
		  tmp._type = Float;
		  tmp._u.f = (float)(b._u.i);
		  if (abs(tmp._u.f - a._u.f) > 1e-6) {
		    return false;
		  } else {
		    return true;
		  }
	...
	bool Config::deep_async(Config& a, const Config& b) {
	  const float EPSINON = 0.000001;
	  bool result = true;
	  if (a._type != b._type) {
	    if ((a._type == Int && b._type == Float) ||
	   ...
	   if (a._u.b  != b._u.b) {
	    ...
	    if (((a._u.f >= -EPSINON) && (a._u.f <= EPSINON)) &&
	...
	Config Config::diff(const Config& a, const Config& b) {
	  const float EPSINON = 0.000001;
	  Config res = Config::object();
	  const char *a_ = "a";
	  const char *b_ = "b";
	  ...
	  if (a._type != b._type) {
	   ...
	   if (a._u.b  != b._u.b) {
	    ...
	    if (!deep_eq(a, b)) {
	      ...
	      auto&& b_object = b.as_object()._impl;
	      for (auto&& p: b_object) {
		auto it = a_object.find(p.first);
		if (it == a_object.end()) continue;
		auto tmp = diff(it->second._value, p.second._value);
		if (tmp.object_size() > 0)
		  res[p.first] = diff(it->second._value, p.second._value);
	      }
	    }
	  }
	  return res;
...
namespace configuru
{
	...
	struct Writer
	{
		...
		bool write_value_with_hiden(unsigned indent, const Config& config,
							  bool write_prefix, bool write_postfix)
		{
			if (config.is_hiden()) return true;
			if (_options.allow_macro && config.doc() && config.doc() != _doc) {
				dump_file(config.doc()->filename, config, _options);
				_out += "#include <";
				_out += config.doc()->filename;
			...
			if (config.is_null()) {
				...
				if (config.array_size() == 0 && !has_pre_end_brace_comments(config)) {
					...
					auto&& array = config.as_array();
					for (size_t i = 0; i < array.size(); ++i) {
						if (!write_value_with_hiden(indent + 1, array[i], false, true)) {
						      if (_compact) {
							      if (i + 1 < array.size()) {
								      _out.push_back(',');
							      }
				...
				if (config.object_size() == 0 && !has_pre_end_brace_comments(config)) {
					...
					write_object_contents_with_hiden(indent + 1, config);
					write_indent(indent);
					_out.push_back('}');
				}
			} else {
		...
		void write_object_contents_with_hiden(unsigned indent, const Config& config)
		{
			if (config.is_hiden()) return;
			// Write in same order as input:
			auto&& object = config.as_object()._impl;
			...
			for (auto it=object.begin(); it!=object.end(); ++it) {
				if (it->second._value.is_hiden()) continue;
				pairs.push_back(it);
				if (align_values) {
					longest_key = (std::max)(longest_key, it->first.size());
				}
			...
			for (auto&& it : pairs) {
				...
				write_value_with_hiden(indent, value, false, true);
				if (_compact) {
					if (i + 1 < pairs.size()) {
						_out.push_back(',');
					}
	...
	std::string dump_string_with_hiden(const Config& config, const FormatOptions& options)
	{
		Writer w(options, config.doc());
		if (options.implicit_top_object && config.is_object()) {
			w.write_object_contents_with_hiden(0, config);
		} else {
			if (!w.write_value_with_hiden(0, config, true, true)) {
			      if (options.end_with_newline && !options.compact())
			      {
				      w._out.push_back('\n'); // Good form
			      }
